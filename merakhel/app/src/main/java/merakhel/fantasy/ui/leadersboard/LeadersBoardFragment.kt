package merakhel.fantasy.ui.leadersboard
...
import merakhel.fantasy.TeamPreviewActivity
import merakhel.fantasy.R
import merakhel.fantasy.databinding.FragmentLeadersBoardBinding
...
import merakhel.fantasy.ui.leadersboard.models.LeadersBoardModels
...
class LeadersBoardFragment(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter!!.onItemClick = { objects ->
            ...
            if(objects.userId == MyPreferences.getUserID(requireActivity())){
                getPoints(objects.teamId)
            }else
            if(matchObject.status!=BindingUtils.MATCH_STATUS_UPCOMING) {
                getPoints(objects.teamId)
            }else {
                MyUtils.showToast(requireActivity() as AppCompatActivity,"You cannot see other players teams, until match started")
            }
    ...
    fun getLeadersBoards() {
       ...
       // customeProgressDialog.show()
        ...
        models.contest_id =""+contestObject!!.id
        models.match_id =""+matchObject.matchId
        WebServiceClient(requireActivity()).client.create(IApiMethod::class.java).getLeaderBoard(models)
            .enqueue(object : Callback<UsersPostDBResponse?>{
                override fun onFailure(call: Call<UsersPostDBResponse?>?, t: Throwable?) {
