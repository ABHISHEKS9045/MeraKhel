package merakhel.fantasy.ui.createteam.adaptors
   ...
   val playerType:Int
    ...
    override fun onBindViewHolder(parent: RecyclerView.ViewHolder, viewType: Int) {
        ...
        if (matchObject.teamAInfo!!.teamId == objectVal.teamId) {
            viewHolder?.teamName?.setBackgroundColor(context.resources.getColor(R.color.Skyblue))
            viewHolder?.teamName?.setTextColor(context.resources.getColor(R.color.Skyblue))
            when(playerType){
                1->
                Glide.with(context)
            ...
            viewHolder?.teamName?.setBackgroundColor(context.resources.getColor(R.color.colorPrimaryDark))
            viewHolder?.teamName?.setTextColor(context.resources.getColor(R.color.Skyblue))
            when(playerType){
                1->
                    Glide.with(context)
                        ...
                        .into(viewHolder?.playerImage as ImageView)
                0->
                    Glide.with(context)
                        .load(objectVal.playerImage)
...
//        })
        ...
        if (objectVal.isPlaying11 && matchObject.isLineup) {
            ...
            viewHolder?.anouncedIndicatorText?.setTextColor(context.resources.getColor(R.color.reddark))
        } else {
...
//            viewHolder?.linearTradesStatus.setBackgroundColor(context.resources.getColor(R.color.grey))
        }
