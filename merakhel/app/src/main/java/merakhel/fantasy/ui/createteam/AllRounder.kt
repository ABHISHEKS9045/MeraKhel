            ...
            if(objects.isSelected) {
                ...
                if(!CreateTeamActivity.isAllPlayersSelected!!) {
                    if(count < CreateTeamActivity.MAX_ALL_ROUNDER[1]) {
                        ...
                        MyUtils.showToast(requireActivity() as AppCompatActivity,"MAX ALLOWED is "+ CreateTeamActivity.MAX_ALL_ROUNDER[1])
                    }
                }else {
    ...
    private fun isMinimumPlayerSelected(): Boolean {
        if(CreateTeamActivity.totalPlayers>= CreateTeamActivity.MAX_PLAYERS_FROM_TEAM) {
            if (CreateTeamActivity.COUNT_WICKET_KEEPER < CreateTeamActivity.MAX_WICKET_KEEPER[0]) {
                ...
                MyUtils.showToast(requireActivity() as AppCompatActivity,"Minimum "+ CreateTeamActivity.MAX_BATSMAN[0]+" "+"BatsMan Required")
                return false
